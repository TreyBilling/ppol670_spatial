...
Rasters are commonly used to store images from satellites like climate/weather patterns. To make things more concrete, let's pull in some example rasters from the `raster` package, which is the workhorse package for rasters in `R`. The `temperautre` object holds a raster for mean temperature each of the 12 months, but I'm going to pull out just the layer for April. Note that temperature is stored as Celsius * 10, so I divided by 10 to rescale.
